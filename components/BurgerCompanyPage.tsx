...
const BurgerCompanyPage: React.FC<BurgerCompanyPageProps> = ({ user }) => {
    ...
    const [config, setConfig] = useState<BurgerConfig>({ ...initialConfigState, email: user.email });
    ...
    const handleCreateNew = () => {
        setConfig({ ...initialConfigState, email: user.email });
        setView('form');
        setError(null);
    };
            ...
            <form onSubmit={handleSave} className="space-y-8 max-w-5xl mx-auto">
                ...
                <section className="bg-gray-700/30 p-6 rounded-xl border border-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">Email do Proprietário</label>
                            <input 
                                type="text" name="email" value={config.email} onChange={handleChange} required readOnly
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-gray-800 p-4 rounded-lg border border-gray-600">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                                {config.CAIXA.map(email => (
                                    <span key={email} className="px-2 py-1 bg-blue-900/40 border border-blue-800 rounded-md text-xs text-blue-200 flex items-center gap-1">{email} <button type="button" onClick={() => handleArrayRemove('CAIXA', email)} className="text-red-400 hover:text-red-300 ml-1"><XCircleIcon className="w-3 h-3"/></button></span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-800 p-4 rounded-lg border border-gray-600">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                                {config.GARCOM.map(email => (
                                    <span key={email} className="px-2 py-1 bg-yellow-900/40 border border-yellow-800 rounded-md text-xs text-yellow-200 flex items-center gap-1">{email} <button type="button" onClick={() => handleArrayRemove('GARCOM', email)} className="text-red-400 hover:text-red-300 ml-1"><XCircleIcon className="w-3 h-3"/></button></span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-800 p-4 rounded-lg border border-gray-600">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                                {config.DELIVERY.map(email => (
                                    <span key={email} className="px-2 py-1 bg-green-900/40 border border-green-800 rounded-md text-xs text-green-200 flex items-center gap-1">{email} <button type="button" onClick={() => handleArrayRemove('DELIVERY', email)} className="text-red-400 hover:text-red-300 ml-1"><XCircleIcon className="w-3 h-3"/></button></span>
                                ))}
                            </div>
                        </div>
                    </div>
