    ...
    <div className="w-full bg-gray-800/80 rounded-xl border border-gray-700/50 overflow-hidden shadow-lg relative">
      ...
      <div className="relative flex items-center">
        <button type="button" onClick={prevDay} className="absolute left-0 z-10 p-1 h-full bg-gradient-to-r from-gray-900 via-gray-900/80 to-transparent text-gray-400 hover:text-white flex items-center justify-center transition-colors">
          <svg className="w-6 h-6 drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div ref={scrollRef} className="flex overflow-x-auto px-10 py-4 gap-3 custom-scrollbar snap-x w-full">
          {days.length === 0 ? (
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 w-16 h-20 rounded-2xl border flex flex-col items-center justify-center transition-all snap-center focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${
                    isSelected 
                      ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-500/20 scale-105 relative z-0' 
        ...
        <button type="button" onClick={nextDay} className="absolute right-0 z-10 p-1 h-full bg-gradient-to-l from-gray-900 via-gray-900/80 to-transparent text-gray-400 hover:text-white flex items-center justify-center transition-colors">
          <svg className="w-6 h-6 drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>
    </div>
  );
